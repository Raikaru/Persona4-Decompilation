...
# Data addresses the relocation scan cannot derive on its own. Two cases occur:
# the only references live in functions that are still INCLUDE_ASM (so there is
# no relocation to learn from), and the symbol's own NAME disagrees with its true
# address, which makes the cross-file consistency check drop it as a conflict --
# `D_007242A0` really lives at 0x007642a0, for instance.
#
# Every entry must carry an `evidence:` note saying how the address was
# established, and is re-checked against the retail image by
# tests/test_recovered_symbols.py. This is the data counterpart of the curated
# function names in config/symbol_names.txt; without it, regenerating the
# recovered symbols silently drops a link-eligible object.
CURATED_DATA = REPO / "config" / "symbol_data_addrs.txt"
CURATED_LINE = re.compile(
...
def curated_data_addresses() -> dict[str, int]:
    ...
    out: dict[str, int] = {}
    if not CURATED_DATA.exists():
        return out
    for number, line in enumerate(
            CURATED_DATA.read_text(encoding="utf-8").splitlines(), 1):
        if not line.strip() or line.lstrip().startswith("//"):
            continue
        match = CURATED_LINE.match(line)
        if not match:
            raise SystemExit(f"{CURATED_DATA}:{number}: malformed entry: {line!r}")
        if "evidence:" not in match.group("rest"):
            raise SystemExit(
                f"{CURATED_DATA}:{number}: {match.group('name')} has no evidence: note")
...
def main() -> None:
    ...
    curated = curated_data_addresses()
    # A curated entry wins over a scanned one only when the scan produced
    # nothing; if both exist and disagree, that is a real contradiction and the
    # curated file is stale, so fail loudly rather than pick a side.
    for name, address in curated.items():
        if name in emitted and emitted[name] != address:
            raise SystemExit(
                f"recover_symbols: {name} is {emitted[name]:#010x} by relocation "
                f"evidence but {address:#010x} in {CURATED_DATA.name}; "
                "fix or drop the curated entry")
    lines = [
        ...
        "// symbol with conflicting evidence is dropped). Entries the scan",
        f"// cannot derive are curated in {CURATED_DATA.name}. Do not hand-edit.",
        f"_gp = {gp:#010x}; // type:gp",
    ]
    merged = dict(curated)
    merged.update(emitted)
    for name, address in sorted(merged.items(), key=lambda kv: (kv[1], kv[0])):
