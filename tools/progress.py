...
STATUS_BEGIN = "<!-- STATUS:BEGIN (generated by tools/progress.py --update-readme) -->"
STATUS_END = "<!-- STATUS:END -->"


def render_status(metrics: dict, recovery: dict | None) -> str:
    """Build the README status table from the metrics that were just verified.

    The table used to be maintained by hand and drifted badly: it claimed 1,314
    matching functions and "99.6% of first-party tracked" while the metrics said
    3,419 and 67.5%. Anything a reader could check must therefore come from the
    endpoints, and matching must be reported separately from recovery so a match
    count is never read as a count of finished, readable source.
    """
    total = metrics["total"]
    matching, linked = metrics["matching"], metrics["linked"]
    hashes = metrics["hashes"]
    rows = [
        "| Item | Verified value |",
        "| --- | --- |",
        f"| Retail executable | `SLUS_217.82`; SHA-1 `{hashes['retail_sha1']}` |",
    ...
    if recovery:
        ...
        rows += [
            f"| First-party matched, scored for recovery | {scored:,} |",
            f"| \u2014 NAMED (not a `func_<address>` placeholder) | {recovery['named']:,} "
            f"({recovery['named_percent']}%) |",
            f"| \u2014 TYPED (no raw-offset or `M2C_` access) | {recovery['typed']:,} "
            f"({recovery['typed_percent']}%) |",
            f"| \u2014 DOCUMENTED (prose, or trivially self-evident) | {recovery['documented']:,} "
            f"({recovery['documented_percent']}%) |",
            f"| \u2014 still carrying decompiler local names | {recovery['decompiler_residue']:,} "
...
def update_readme(path: Path, body: str) -> None:
    text = path.read_text(encoding="utf-8")
    if STATUS_BEGIN not in text or STATUS_END not in text:
        raise ProgressError(f"{path} has no STATUS:BEGIN/STATUS:END block to fill")
    head, rest = text.split(STATUS_BEGIN, 1)
    _stale, tail = rest.split(STATUS_END, 1)
    path.write_text(f"{head}{STATUS_BEGIN}\n{body}{STATUS_END}{tail}", encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--update-readme", type=Path, metavar="PATH",
                        help="rewrite the STATUS block in this markdown file")
    parser.add_argument("--recovery-report", type=Path, metavar="PATH",
                        help="recovery_quality.py --json output, to include the recovery rows")
    args = parser.parse_args()
    try:
        windows = load_windows()
        if args.validate_dir is not None:
            if args.report or args.linked_report or args.write_dir or args.json:
                raise ProgressError("--validate-dir cannot be combined with report, write, json, or readme options")
        ...
        if args.update_readme:
            if linked_report is None:
                raise ProgressError("--update-readme requires a successful --linked-report")
            recovery = load_json(args.recovery_report, "recovery report") if args.recovery_report else None
            update_readme(args.update_readme, render_status(metrics, recovery))
            print(f"status table written to {args.update_readme}")
        if args.json:
