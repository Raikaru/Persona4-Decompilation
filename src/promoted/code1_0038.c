...
/* 2026-09-20 deficit probe: baseline 727wd / fnalign 143 edits (+2 reloc-only), retail 846 / object 785 (deficit 61, -7.2% OUTSIDE). deficit_scan opcode delta retail-minus-object: nop +15, andi +11, mfc1 +6, cvt.w.s +6, lui +5, b +4, sub.s +3, or +3. Three retail-only runs of 18, each paired against 1 object instr with 2 nearby -> ABSENT by length rule (18 <= 61, no run longer than deficit so no CROSS): 0x38459c-0x3845e4 (retail[391:409] c.ole/bc1t/nop/cvt/mfc1/nop/andi/b/nop/sub/cvt/mfc1/lui/or/andi/bltz/nop/mtc1 vs mtc1 $s0, net +17), 0x384a70-0x384ab8 (retail[700:718] same 18 vs mtc1 $s0, net +17), 0x384bcc-0x384c14 (retail[787:805] same 18 vs mtc1 $s0, net +17); subtotal 51 net of 61. Remaining 10 net are scattered singletons (no run longer than 3): retail[56:59] net +2 / retail[65:66] +1 / retail[120:123] +2 / retail[129:130] +1 / retail[150:153] +2 / retail[314:323] +8 with retail[311:313] -5 (first-site tail-merge pair nets +3) / retail[335:336] +1 / retail[388:390] +1 / retail[421:422] +1 / retail[473:474] +1 / retail[528:531] +3 / retail[591:592] +1 / retail[646:647] +1 / retail[697:699] +1 / retail[730:731] +1 / retail[784:786] +1 / retail[817:818] +1 offset by sixteen object-singleton inserts and small replaces (-1 each, total -19), nets +10; all ABSENT by length rule. Honest levers tried this session and rejected: operand-swap (scale*192.0f on sites 2/4) ties 727wd / 785 / 143e; separate f32 p0-p3 temps ties 728wd / 785 / 214e; opt_common_subs off reaches 866 vs 848 (+2.1% inside) but regresses to 814wd / 1178e (steering, not adopted); opt_propagation off regresses to 771wd / 773. Root cause is integer CSE: object computes (u8)(u32)(192.0f*scale) once into $s0 and reuses it three times, retail reconverts each time; plus first-site tail-merge (object shares one andi $s0 vs retail duplicates andi $a3). No honest C spelling tested defeats it without blowing up edits, so floor stays banked and words stay non-comparable per handoff 7y. */
    ...
    *(u16 *)(state + 4) = (*(u16 *)(state + 4) + 1) % 10;
    {
        ...
        for (i = 0; i < 15; i++) {
            ...
            fy = *(f32 *)(entry + 4);
            ff = *(f32 *)(entry + 8);
            s = func_0044b7b0((fGpffff81e0 * (ff * 10.0f + (f32)*(u16 *)(state + 4))) / 10.0f);
            t = (s + 1.0f) / 2.0f;
            if (*(u16 *)(state + 4) == 0) {
                f32 rnd;
                f32 v;
                rnd = (f32)(u32)(func_003b7060() & 0xFFF);
