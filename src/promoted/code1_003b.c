...
/* measured: batch method, 3 batches / 16 candidates this session, all measured via probe_variants + fnalign --candidate against the guarded body (object 42 / retail 48, -12.5%, OUTSIDE the +-3% band). fnalign reports 14 edits with exactly two pure delete runs totalling the missing 6 instrs, one whole out-of-line construct: 1 of the 6 is retail[33] b .L003BCEF0 at 0x003BCED4 (shared-return branch over the out-of-line block; its delay addu $v0,$s4,$s2 at 0x003BCED8 aligns with the object inline addu so is not missing) and 5 of the 6 are retail[35:40] jal func_003bc620 at 0x003BCEDC + nop at 0x003BCEE0 + b .L003BCEB0 at 0x003BCEE4 with delay addu $s2,$s2,$v0 at 0x003BCEE8 + nop at 0x003BCEEC (out-of-line call block reached by the loop bnez $v0,0x003BCEA4->0x003BCEDC). Object inlines the call with beqz-skip (jal at object[21:22] + addu $s2,$s2,$v0 at object[23:24]) and falls straight into the tail. Batch1 volatile-double + call shapes (goto/switch/ternary/for, A-F) best nd26 vs baseline nd25; batch2 cached-double + loop forms (G-K) G hits 48/48 but only via spurious dsll32/dsra32 for (s64)0<count on cached s32 (retail is bare slt $at,$zero,$v0 on the single lw) with nd26; batch3 cached-double + call shapes (goto/switch/ternary/continue/else, L-P) all inline beqz-skip nd26-35. No spelling moves the call out-of-line: placement floor, not six stray instrs. */
#pragma schedule on
#pragma no_branch_likely on
#pragma opt_rebuildconditionals off
#pragma opt_propagation off
