...
/* measured: out-of-line cold block is real b210 codegen, not missing logic. func_003bc880 does the idiom twice at retail: loop 1 bnez 0x003BC8D4->0x003BC908 (counter-inc cold, b back to 0x003BC8E0) and loop 2 bnez 0x003BC958->0x003BC984 (jal func_003bc400 cold, b back to 0x003BC960 with the accumulate in the delay slot), each with a shared-return b over the cold. Object here inlines the call behind a beqz-skip (jal at object[21:22] + addu at object[23:24]), so the shortfall is one out-of-line construct totalling the 6 (object 42 / retail 48): retail[33] b at 0x003BCED4 over the cold plus retail[35:40] jal/nop/b/addu/nop at 0x003BCEDC-0x003BCEEC reached from the loop bnez at 0x003BCEA4. Negative: 23 spellings across guard-as-continuation-test, goto-to-shared-return, and accumulate-inside-cold-vs-after-join all emit the inline beqz-skip, including an explicit goto-next join (object 46 vs 48) which buys its two instructions with an extra lw hole rather than by outlining -- pre-refuted. func_003bc880 has no matched C in the tree (INCLUDE_ASM here and in cw119; only M2C/plain-if archive sources exist), so there is no worked example to copy. Open lead: the idiom appears in retail twice and has never been reproduced once here, so this is a compiler-behaviour question for a minimal b210 test case rather than more source respellings. */
#pragma schedule on
#pragma no_branch_likely on
#pragma opt_rebuildconditionals off
#pragma opt_propagation off
