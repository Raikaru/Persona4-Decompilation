...
extern s32 datCalcGetHp(s32 arg0);
...
void func_00200770(u8 *arg0)
{
    ...
    if ((temp_3 >= temp_4) || (temp_3 == 0)) {
        ...
        if (((func_001bc7f0() & 0xFFFF) == 1) && (*(s32 *)temp_17 != 0)) {
            ...
            mdlScale(*(u8 **)temp_17, &frame.sp120.x, 2);
...
loop_17_body:
    ...
    if ((temp_4_2 != 0) && (datCalcIsDead(temp_4_2, 0) == 0)) {
        func_00194ff0(var_17, sp40, sp30, NULL);
        btlUnitSetPos(var_17, sp40);
        btlUnitSetRot(var_17, sp30);
...
void func_00201a80(u8 *arg0) {
    ...
    *(u16 *)(temp_5_2 + 0xA0) = (u16) (*(u16 *)(temp_5_2 + 0xA0) + 1);
    ...
    if ((temp_4 != 0) && (datCalcChkBadStatus(temp_4, 0x100000) != 0)) {
        *(s32 *)(arg0 + 0x34) = 1;
        return;
    }
    *(s32 *)(arg0 + 0x34) = 0;
...
s32 func_00202480(u8 *arg0)
{
    ...
    switch (*(u8 *)(arg0 + 4)) {
    case 0:
        var_17 = (s16)datCalcGetHp(*(s32 *)(temp_16 + 0xA64));
...
s32 func_00202640(u8 **arg0)
{
    ...
    temp_16 = *arg0;
    var_f20 = (f32)(u32)datCalcGetHp(*(s32 *)(temp_16 + 0xA64));
...
/* Rotated battle-panel strip with an optional transparent extension.
 * Measured b210 O2 with scoped opt_propagation off: 940 executable bytes,
 * 14 resolved relocations and four zero alignment bytes. The corner record
 * and staged Y sum preserve retail's repeated calculations and FP order.
 * All 145 other functions and allocated data are unchanged.
 * See docs/probe_archive/Battle_strip_00204dc0_20260924.md. */
...
#pragma push
#pragma opt_propagation off
void func_00204dc0(s32 index, f32 inputX, f32 inputY, f32 depth, f32 angle, s32 extend)
{
    f32 vertices[4][16];
    ...
    f32 displacement;
    {
        void (**initialState)(u32, u32);
        initialState = D_00887300;
        initialState[0](1, 0);
    }
    reciprocal = 1.0f / *(f32 *)(func_00457120() + 0x80);
    depth = D_008872F8[0] - depth;
    radians = fGpffff836c * angle;
    cosine = cosf(radians);
    sine = sinf(radians);
    width = 86.0f;
    x = inputX + 97.0f;
    y = inputY + ((0.0f + 312.0f) + 15.0f * (f32)((s16)index - 3));
    margin = 64.0f;
    halfWidth = width + margin;
    zero = 0.0f;
    {
        struct { f32 x, y; } offset;
        offset.x = zero + halfWidth;
        offset.y = 8.0f;
        sinOffset = offset.y * sine;
        vertices[0][0] = x + offset.x * cosine - sinOffset;
        cosOffset = offset.y * cosine;
        displacement = y + offset.x * sine;
        vertices[0][1] = displacement + cosOffset;
        vertices[0][2] = depth;
        offset.x = zero - halfWidth;
        product = offset.x * cosine;
        leftX = x + product;
        vertices[1][0] = leftX - sinOffset;
        product = offset.x * sine;
        leftY = y + product;
        vertices[1][1] = leftY + cosOffset;
        vertices[1][2] = depth;
        offset.y = -8.0f;
        sinOffset = offset.y * sine;
        vertices[3][0] = leftX - sinOffset;
        cosOffset = offset.y * cosine;
        vertices[3][1] = leftY + cosOffset;
        vertices[3][2] = depth;
        offset.x = zero + halfWidth;
        vertices[2][0] = x + offset.x * cosine - sinOffset;
        displacement = y + offset.x * sine;
        vertices[2][1] = displacement + cosOffset;
        vertices[2][2] = depth;
    }
    vertices[0][8] = 27.0f;
    vertices[0][9] = 27.0f;
    ...
    if (extend != 0) {
        ...
        zero = 0.0f;
        {
            ...
            offset.x = zero + halfWidth;
            offset.y = zero;
            vertices[0][0] = x + offset.x * cosine - offset.y * sine;
            vertices[0][1] = y + offset.x * sine + offset.y * cosine;
            offset.x = offset.y - halfWidth;
            offset.y = zero;
            product = offset.x * cosine;
            leftX = x + product;
            vertices[1][0] = leftX - offset.y * sine;
            product = offset.x * sine;
            leftY = y + product;
            vertices[1][1] = leftY + offset.y * cosine;
            offset.y = -800.0f;
            sinOffset = offset.y * sine;
            vertices[3][0] = leftX - sinOffset;
            cosOffset = offset.y * cosine;
                   ...
                   u8 opacity, s32 highlighted, s32 showDetail)
{
    ...
    func_00201650(glyphs, 13, 0x2A, 33.0f + x, 12.0f + y,
                  tone, tone, tone, opacity);
    level = (u8)datPersonaGetLevel(persona);
    digit_y = 5.0f + y;
    digit_x = 63.0f + x;
    ...
    *(s16 *)(work + 0x14) = 3;
    ...
    case 4:
        if (func_001f0620(*(u8 **)(work + 0x178), 2) == 0) {
            if (datCalcChkBadStatus(*(s32 *)(*(u8 **)(*(u8 **)(work + 0x178) + 0x30) + 0xA64), 8) != 0) {
    ...
    if (*(u16 *)state == 1 && !(*(u32 *)(iGpffffb3ac + 0xC) & 0x10000)) {
        ...
        if (*(u8 **)(state + 0x178) != 0 &&
            ...
            *(u8 **)(*(u8 **)(*(u8 **)(state + 0x178) + 0x30) + 0xA64) != 0) {
            f32 pulse;
            if (datCalcIsLowHp(*(u8 **)(*(u8 **)(*(u8 **)(state + 0x178) + 0x30) + 0xA64))) {
                pulse = sinf(fGpffff8478 * fGpffffb478);
                fGpffffb478 += 2.0f;
...
void func_0020e420(u8 *arg0, f32 fparg0, f32 fparg1, s32 arg1, s32 arg2, u32 arg3)
{
    ...
    if (c0 != 0) {
        sprintf(sp90, (const char *)&iGpffffa580, arg1);
        count = strlen(sp90);
        if (arg2 != 0) {
...
f32 func_0020e5c0(s32 arg0, s32 arg1, s32 arg2, s32 arg3)
{
    ...
    if (arg0 < arg2) {
        ...
        switch (arg3) {
        case 1:
            temp_f0 = sinf(fGpffff84a4 * temp_f0);
            goto done;
        case 2:
            temp_f0 = 1.0f - cosf(fGpffff84a4 * temp_f0);
            goto done;
        default:
            goto done;
        }
...
/* 820/832 bytes with 26 resolved relocations and three exact lookup tables.
   The caller passes a packed four-byte color. Independent complete coordinate
   array views preserve the row-offset lifetime across the dimension queries.
   See docs/probe_archive/Battle_panel_0020f730_20260922.md. */
// FUN_0020F730
void func_0020f730(u8 *work, u8 *state, s32 packedColor, s32 index)
{
    f32 corners[2][2] = {
        {-98.0f, -92.0f},
        {98.0f, 92.0f}
    };
    /* One interleaved coordinate block, with complete X-origin and
       Y-origin array views. Each row advances eight bytes inside its
       actual array; the leading firstX overlays the first coordinate. */
    union {
        f32 fromX[6];
        struct { f32 firstX; f32 fromY[5]; } y;
    } offsets[1] = {{{15.0f, 54.0f, 27.0f, 25.0f, 57.0f, 14.0f}}};
    ...
    if (index != 0) {
        scaleY = fGpffff8210 * *(f32 *)(state + 8);
        half = index / 6;
        flipX = 1 - (((index / 3) + half) & 1);
        flipY = 1 - half;
        if (flipX == 0) {
            sign = 1;
        } else {
        ...
        rowOffset = part * (2 * sizeof(f32));
        width = func_00201950(work, 10, sprite);
        adjustmentX = (f32)flipX * width;
        scaleX = fGpffff8210;
        adjustmentX = adjustmentX / scaleX;
        offsetX = *(f32 *)((u8 *)&offsets[0].fromX + rowOffset);
        drawX = scaleX * (drawX * (offsetX + adjustmentX));
        height = func_00201990(work, 10, sprite);
        adjustmentY = (f32)flipY * height;
        adjustmentY = adjustmentY / scaleY;
        offsetY = *(f32 *)((u8 *)&offsets[0].y.fromY + rowOffset);
        drawY = scaleY * (drawY * (offsetY + adjustmentY));
        func_002012d0(work,
                     (0.0f + (f32)0x117) + fGpffff8210 * corners[flipX][0],
                     (0.0f + 229.0f) + scaleY * corners[flipY][1]);
        blue = ((u8 *)&packedColor)[2];
        green = ((u8 *)&packedColor)[1];
        func_00201650(work, 10, sprite, drawX, drawY,
                      ((u8 *)&packedColor)[0], green, blue, ((u8 *)&packedColor)[3]);
        if (index < *(s16 *)(state + 2)) {
            func_00201650(work, 10, sprite, drawX, drawY,
                          ((u8 *)&packedColor)[0], green, blue, 255);
        }
    }
}
// FUN_0020FA70
...
void func_0020ff00(u8 *glyphs, u8 *transition)
{
    ...
    if (frame < 0x23) {
        ...
        lateBand = 1.0f - cosf(fGpffff84a4 * ((f32)(frame - 0x23) / 10.0f));
    } else {
        lateBand = 1.0f;
    }
    if (!(lateBand <= 0.0f)) {
    ...
    if (frame < 0x19) {
        ...
        earlyBand = 1.0f - cosf(fGpffff84a4 * ((f32)(frame - 0x19) / 10.0f));
    } else {
        earlyBand = 1.0f;
    }
    if (!(earlyBand <= 0.0f)) {
    ...
    if (frame < 0xA) {
        ...
        narrowIntro = sinf(fGpffff84a4 * ((f32)(frame - 0xA) / 10.0f));
    } else {
        narrowIntro = 1.0f;
    }
    if (frame < 0x28) {
        ...
        narrowExpansion = 1.0f - cosf(fGpffff84a4 * ((f32)(frame - 0x28) / 10.0f));
    } else {
    ...
    if (frame < 0) {
        ...
        diagonalIntro = sinf(fGpffff84a4 * ((f32)frame / 10.0f));
    } else {
        diagonalIntro = 1.0f;
    }
    if (frame < 0x34) {
        ...
        diagonalExpansion = 1.0f - cosf(fGpffff84a4 * ((f32)(frame - 0x34) / 10.0f));
    } else {
