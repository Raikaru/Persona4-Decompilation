...
/* measured: live object 1932B/window 1984B, normalized_diff 465 (installed guard below; first real floor, replaces bare ASM). Structure from ghidra adapted to the file's MATCHED 0049b2b0 idiom (guard polarity corrected to >= per retail sltu+beqz, COP2 unpack/pack/webs transcribed to __asm__ with % slots, adda/madd leading-zero idioms, u32 piVar15/14). Open walls: scattered saved/FP allocation ($s0-vs-$s1 class, no systematic rename found), COP2 slot addresses (frame layout), named pextlb/pextlh MMI pair, s128-canonicalization family. Ruled out: u32-mask decoupling (reverses evaluation), s32-mult slot forms (uncompilable). Banked as floor. */
#ifdef NON_MATCHING
