...
/* Named compiler floor: retail's standalone R5900 MMI pextlb/pextlh pair widens
   packed scalar color data before the COP2 block; b210 cannot emit this sequence
   from C. Leave the assembly fallback rather than forcing ordinary-computation asm. */
/* Draft (measured 2026-09-17, source-repo only): probe_variants p3_both (loop+prop) 495 words BEST (p0 523, p1 530, p2 520), fnalign 500/546/466, emitted 2184B/window 2000B (+9.2%% over, 46 instrs over from scalar VU expansion; lower-bound 2184>=1940 but not near 2000). Four-pragma sweep measured, both wins words+edits. wscan OBJ 6 dsll32/dsra32 vs RETAIL 0 (s128 width wall) + MMI + interior VU pipeline, frame -0x120 vs -0x190. More than nominal size and carries fixable width wall, so left as plain INCLUDE_ASM draft, not a floor. See /var/tmp/EffE150/body.c and report.txt for candidate. */
// FUN_0049E150
