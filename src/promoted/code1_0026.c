...
/* decomposition for func_00261560 (left as ASM, 2 over with real expressions — same call as func_0035aff0):
   Retail 0x00261560-0x00262DDF 6272B 1568I, band 1521-1615. Best real v1d 1617I/1430wd/3002ed (+49/+2 over max) with handoff-7r entry shapes (s32)((float)arg0+47.0f*(1.0f-fparg1)) and four 0xE 9th-args ((float)temp_f0+70/69*fparg1, 70,69,70,69 order per c.lt.S f22,1.0/bc1t), s0 reuse via arg0, opt_level3 scoped (baseline v1e 1804I/1591wd -> 1558I/1357wd dummy, +59/+73 for real shapes). 12-arg ABI from prologue: a0->s0(arg0),a1->s4(arg1),a2->s3(arg2 u8),a3->s8(arg3 &0xF,>>4),t0->stack0xCC(arg4),f13->f22(fparg1),f14->f21(fparg2),t1->s6(arg5),t2->s7(arg6),t3->s2(arg7). MAC addrs 0x00261614/18,0x002619EC/F0,0x00261A5C/60,0x00261D00/04,0x00261D70/74,0x00261FDC/E0 (all clear-prime plain a+b*c). 12th-arg f15 0.0f is retail clear-prime value (correct literal, not stub). Stub 0.0f version reached 1558I/1357wd inside but withheld as placeholder per handoff (worse than no body). Next: declaration-order for s0 (var_17 vs sp buffers first) and redundant &0xF/&0xFFFF/(s64) removal to close 2I. */
// FUN_00261560
