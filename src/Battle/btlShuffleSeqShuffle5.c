...
// Measured floor (nd 344, obj 1260/win 1264): b210 keeps the loop-invariant
// unit-pointer mults inside the inner shuffle loop (recomputed per iteration,
// colored $a1/$a0) while retail hoists them to the outer loop body colored
// $4/$5. Writing them outside the loop flips them to callee-saved $s6/$s2 with
// a 0x30-larger frame (measured nd 956+ across all structural variants tried).
// Residual: fix2float FPU register choice ($f2 vs $f1) and copy-loop registers.
#ifdef NON_MATCHING
void func_0037ef40(u8 *arg0)
{
    ...
    temp_18 = ctx->count_1F304;
    if (temp_18 < 6) {
        for (var_20 = 0; var_20 < 6; var_20 += 2) {
            var_19 = temp_18 - 1;
            while (var_19 > 0) {
                var_5 = var_20 * temp_18;
                var_4 = (var_20 + 1) * temp_18;
                temp_17 = arg0 + (var_5 * 0xFB0);
                temp_22 = arg0 + (var_4 * 0xFB0);
                ...
                if ((idx < 0) || (idx >= temp_18)) {
                    func_0046d730(&D_0064EB40[0], 0xCC);
                }
                a = temp_17 + (var_19 * 0xFB0);
                w = a;
                dst = buf1;
                copy_i = 0x1F6;
                do {
    ...
    for (var_20 = 0; var_20 < 3; var_20++) {
        var_19 = ctx->count_1F304 - 1;
        while (var_19 > 0) {
            var_4 = var_20 * temp_18;
            temp_17 = arg0 + (var_4 * 0xFB0);
