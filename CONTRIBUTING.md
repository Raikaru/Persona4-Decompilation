...
`make split` produces bulk disassembly and build assets; it is not sufficient
to recreate the per-function `INCLUDE_ASM` files. `make regenerate-asm` runs
`tools/regenerate_asm.py --fresh` before compilation: it refuses unexpected
changes to existing generated fallbacks, removes only paths classified as
generated by `config/generated_asm.json`, splits the validated private retail
ELF in scratch space, and requires every regenerated file's SHA-256 to match
the manifest. Manual exceptions remain in place. `--check` performs the same
scratch reproduction check without writing fallback output files:

```sh
P4_RETAIL_ELF="/path/to/SLUS_217.82" python tools/regenerate_asm.py --check
```

Use `orig/SLUS_217.82`, `P4_RETAIL_ELF`, or `--retail PATH` for this tool;
unlike the compiler tools, it does not read ignored local configuration files.
Install the exact public Python dependencies from `requirements-python.txt`.
R5900 binutils and proprietary compilers are needed for the subsequent
build/verification, not regeneration. The split configuration, symbols, canonical function
map, extraction tools, and manifest recipes/corrections are tracked inputs;
old split or fallback outputs are not inputs.

When changing a fallback, make the change reproducible through an explicit
...
ELF can reproduce retail disassembly, not the C attempts, measurements, or
human recovery decisions. Neither archive cleanup nor a broad ASM ignore rule
may discard that work.
