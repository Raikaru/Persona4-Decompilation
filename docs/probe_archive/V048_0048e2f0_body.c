/* archive: candidate object 0 (M2C compile-error candidate; normalized_diff unavailable); retail window 2256 bytes; normalized_diff unavailable because candidate does not compile. Conversion casts: int->float sites 0x154, 0x1FC, 0x260, 0x674: `(f32)(u32)value` at each. Ruled out: COP1 accumulator chain (`adda.s`/`madd.s`) plus COP2/VU packet operations; documented floor. */
