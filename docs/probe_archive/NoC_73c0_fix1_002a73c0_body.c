 ...
 * Lane: NoC_73c0_fix1
 * Change: removed the `result` integer local and wrote the conversion directly
 *   into the s16 store:
 *     *(s16 *)(q + 0x12) = (s16)((u32)temp + 30);
 *   This made the compiler sink the float-to-u32 conversion to after the
 *   func_002a7330 call, save `temp` in $f22, and shrink the frame to 0x90
 *   (3 saved GPRs + f20-f22). Result: obj 792B / window 848B,
 *   normalized_diff 497, fndiff 183 — worse than the named-local version.
 *   Confirms a named local is required to hold the conversion result across
 *   the call.
 */
