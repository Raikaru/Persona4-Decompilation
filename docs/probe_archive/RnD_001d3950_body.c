// object_size 500, window 512, normalized_diff 13, differing offsets 104 108 112 116 160 164 168 172 176 180; instruction deficit 3; classification aggregate-load register/order plus table-load FPU order; ruled out movz/movn, COP1 accumulator, standalone MMI, framed tail-jump, sd-vs-sq callee-save floors
u32 func_001d3950(u8 **param_1)
{
    ...
    extern FormationPosition D_00881430;
    ...
    while (current != NULL) {
        if (((*(u32 *)(current + 0x9c) & 8) != 0) &&
            ...
            if (*(u8 **)(*param_1 + 0x30) == current) {
                scratch.position.whole = *(u64 *)&D_00881430;
                scratch.position_z = D_00881438.x;
            } else {
