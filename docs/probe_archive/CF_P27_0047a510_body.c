/* Best CF_P27 attempted body for func_0047a510.
 * Measurement: object 440B/window 448B, normalized_diff 28.
 * Retail registers: saves $ra,$s3,$s2,$s1,$s0; $a4=list, $a3=container,
 * $a2=count, $a5=index, $a1=key, $s3=entry, $s1=value, $s2=temp,
 * $s0=destination. Residual: two candidate count remasks and the resulting
 * shifted pointer/callback block; callback argument materialization differs.
 * Tried: generated pseudocode, declaration/local-order permutations, pointer
 * load variants, u16/s32/u32 counter shapes, signed count, callback array,
 * if/else/goto/switch/while branch forms, and optimization pragmas.
 */
